use crate::error::{error_expected_constant, error_invalid_string_utf8};
use crate::{Context, Error, Readable, Reader, Writable, Writer};
use std::borrow::Cow;

pub use crate::error::{
    error_endianness_mismatch, error_invalid_enum_variant, error_invalid_str_utf8,
    error_length_is_not_the_same_as_length_attribute, error_out_of_range_length, error_unsized,
    get_error_kind, ErrorKind,
};
...
#[inline]
pub fn cow_bytes_to_cow_str<E>(bytes: Cow<'_, [u8]>) -> Result<Cow<'_, str>, E>
where
    E: From<Error>,
{
    match bytes {
