...
use crate::error::{error_end_of_input, IsEof};
...
pub trait Reader<'a, C: Context>: Sized {
    ...
    unsafe fn read_bytes_into_ptr(
        ...
        unsafe { self.read_bytes(std::slice::from_raw_parts_mut(output, length)) }
    }
    ...
    unsafe fn peek_bytes_into_ptr(
        ...
        unsafe { self.peek_bytes(std::slice::from_raw_parts_mut(output, length)) }
    }

    #[inline(always)]
    fn skip_bytes(&mut self, mut length: usize) -> Result<(), C::Error> {
        const CHUNK_SIZE: usize = 1024;
        let mut dummy_buffer: [MaybeUninit<u8>; CHUNK_SIZE] =
    ...
    #[inline(always)]
    fn read_bytes_borrowed_from_reader(
        &mut self,
        _length: usize,
    ) -> Option<Result<&[u8], C::Error>> {
        None
    }
            ...
            && (mem::size_of::<T>() == 1 || !self.endianness().conversion_necessary())
        {
            ...
            if let Some(bytes) = self.read_bytes_borrowed_from_reader(bytesize) {
                ...
                unsafe {
                    ...
                    return Ok(RawCopyIter {
                        pointer: std::ptr::NonNull::new_unchecked(pointer as *mut T),
                        end: pointer.add(length),
                    }
                    .collect());
                }
            }
        ...
        V: Readable<'a, C>,
    {
        ...
        if K::speedy_is_primitive()
            && V::speedy_is_primitive()
            && ((mem::size_of::<K>() == 1 && mem::size_of::<V>() == 1)
                || !self.endianness().conversion_necessary())
        {
            let bytesize = length
            ...
            if let Some(bytes) = self.read_bytes_borrowed_from_reader(bytesize) {
                ...
                unsafe {
                    ...
                    return Ok(RawCopyIter {
                        pointer: std::ptr::NonNull::new_unchecked(pointer as *mut Pair<K, V>),
                        end: pointer.add(length),
                    }
    ...
    where
        ...
        T: Readable<'a, C>,
    {
        std::iter::from_fn(move || match self.read_value::<T>() {
